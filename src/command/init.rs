...
pub(crate) async fn run_init(config: &Config) -> Result<(), super::Error> {
    ...
    fs::create_dir_all(&data_dir).await.map_err(|error| {
        super::Error::Init {
    ...
    if !config_file.exists() {
        ...
        fs::write(config_file, DEFAULT_CONFIG).await.map_err(|error| {
            super::Error::Init {
    ...
    tracing::info!("If you want to synchronize hoards between multiple machines, synchronize {}", data_dir.display());
