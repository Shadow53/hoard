...
fn assert_expected_paths(tester: &Tester, expected: &LastPaths) {
    ...
    assert_eq!(
        ...
        expected.hoard("anon_file").expect("hoard should exist").piles,
        "Expected: {:#?}\nReceived{:#?}\n{}", expected, current, tester.extra_logging_output()
    );
    assert_eq!(
...
fn test_last_paths() {
    ...
    assert!(matches!(error, ConfigError::Command(CommandError::Backup(BackupRestoreError::Consistency(CheckerError::LastPaths(LastPathsError::HoardPathsMismatch))))));
    assert_expected_paths(&tester, &first_env_paths);
