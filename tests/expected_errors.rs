...
fn test_invalid_config_extensions() {
    ...
    let error =
        Builder::from_file(&path).expect_err("config file without file extension should fail");
    ...
    let error =
        Builder::from_file(&path).expect_err("config file with bad file extension should fail");
    assert!(matches!(error, BuilderError::InvalidExtension(bad_path) if path == bad_path));
